...
mef_uni <- except_abroad %>% filter(university_name == "MEF ÜNIVERSITESI")
...
mef_faculty_quota <- except_abroad %>% 
  filter(university_name == "MEF ÜNIVERSITESI") %>%  
...
total_eng_faculty_quota <- except_abroad %>% 
  filter(city == "Ä°STANBUL" & faculty_name == "Mühendislik Fakültesi") %>%  
...
ggplot(data=total_eng_faculty_quota, aes(x=reorder(university_name,-total_quota),y=total_quota, fill=time)) +
  geom_bar(colour="black", fill="#e67e22", width=.5, stat="identity",aes(fill=university_name=='MEF ÜNIVERSITESI')) +
  guides(fill=FALSE) +
